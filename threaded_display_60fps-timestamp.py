...
ACQ_PERIOD = 1.0 / 60           # acquisition rate (5 FPS here, change back to 1/60 for 60 FPS)
...
def acquire_loop(paths):
    ...
    for p in paths:
        ...
        timestamp = time.time()  # mark acquisition time
        prep_q.put((img, timestamp))
...
def infer_loop(model, first_shape):
    ...
    while True:
        ...
        # measure lag
        lag_ms = (time.time() - ts) * 1000.0
        lag_stats.add(lag_ms)
