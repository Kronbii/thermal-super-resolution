...
# --------------------- CONFIG ---------------------
DATA_DIR   = "/home/kronbii/repos/thermal-super-resolution/datasets/flir_thermal_x2/val/LR_bicubic/X2"
MODEL_PATH = "/home/kronbii/repos/thermal-super-resolution/checkpoints/_x2/thermal_best.pth"
SCALE      = 2
DEVICE     = "cuda" if torch.cuda.is_available() else "cpu"
ACQ_PERIOD = 1.0 / 60.0           # 60 FPS acquisition
USE_FP16   = True                # safer default (set True if model trained with FP16)
USE_CUDA_GRAPHS = True            # capture static graph if possible
INFER_QUEUE_SIZE  = 4
PREP_QUEUE_SIZE   = 64
WARMUP_ITERS      = 16
...
def preprocess_loop():
    ...
    while True:
        ...
        arr = (img.astype(out_dtype) / out_dtype(255.0))
        tens = torch.from_numpy(arr).unsqueeze(0).unsqueeze(0).to(dtype=torch_dtype).pin_memory()
...
def try_cuda_graph(model, H, W):
    if DEVICE != "cuda" or not USE_CUDA_GRAPHS: return None, None, None
    try:
        static_in = torch.empty((1,1,H,W), dtype=(torch.float16 if USE_FP16 else torch.float32), device=DEVICE)
...
def infer_loop(model, first_shape):
    ...
    if DEVICE == "cuda":
        x = torch.randn(1,1,H,W, device=DEVICE, dtype=(torch.float16 if USE_FP16 else torch.float32))
        with torch.inference_mode():
    ...
    while True:
        ...
        if DEVICE == "cuda":
            ...
            e_tot_s.record()
            e_h2d_s.record()
            if graph: static_in.copy_(cpu_t, non_blocking=True)
            else: d_t = cpu_t.to(DEVICE, non_blocking=True)
            e_h2d_e.record()

            torch.cuda.current_stream().wait_event(e_h2d_e)
            with torch.inference_mode():
                e_inf_s.record()
                if graph: graph.replay(); sr = static_out
                else: sr = model(d_t)
                e_inf_e.record()
        ...
        out = sr.squeeze().detach().float().cpu()
        out = out.clamp(0, 1)  # ensure valid range
